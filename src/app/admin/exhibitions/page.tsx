...
export default function AdminExhibitionsPage() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await PortfolioService.getExhibitions();
    setExhibitions(data);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const remove = async (id: string) => {
    setSaving(true);
    await PortfolioService.deleteExhibition(id);
    await load();
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-headline">Manage Exhibitions</h1>
      </div>
          ...
          <form onSubmit={addExhibition} className="space-y-4">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? (
                ...
                <><Plus className="mr-2 h-4 w-4"/> Add Exhibition</>
              )}
            </Button>
          </form>
        </CardContent>
